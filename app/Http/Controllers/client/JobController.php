...
class JobController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $job = job::where('job_type_id', $job_detail->id)->take(3)->get();
        $job_skills = Job_skill::where('job_post_id', $id)->get();
        return view('client.job.job_detail', compact('job_detail', 'job', 'job_skills'));
    }
    public function cat($id)
    {
        $cat = Job_type::where('id', $id)->first();
        $job_cat = job::where('job_type_id', $id)->get();
        return view('client.job.job_cat', compact('job_cat', 'cat'));
    }
}
